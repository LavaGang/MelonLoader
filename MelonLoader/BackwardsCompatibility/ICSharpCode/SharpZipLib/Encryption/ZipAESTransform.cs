...
internal class ZipAESTransform : ICryptoTransform
{
    ...
    {
        if (blockSize is not 16 and not 32) // 24 valid for AES but not supported by Winzip
        ...
        var key1bytes = pdb.GetBytes(_blockSize);
        var key2bytes = pdb.GetBytes(_blockSize);
