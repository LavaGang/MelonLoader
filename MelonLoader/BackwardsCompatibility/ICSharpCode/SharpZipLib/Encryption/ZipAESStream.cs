...
internal class ZipAESStream : CryptoStream
{
    ...
    */
    ...
    private int ReadAndTransform(byte[] buffer, int offset, int count)
    {
        ...
        while (nBytes < count)
        {
            var bytesLeftToRead = count - nBytes;
            ...
            var lengthToRead = BLOCK_AND_AUTH - byteCount;
    ...
    {
        var copyCount = Math.Min(count, _transformBufferFreePos - _transformBufferStartPos);
    ...
    private int TransformAndBufferBlock(byte[] buffer, int offset, int count, int blockSize)
    {
        ...
        var bufferRequired = blockSize > count;
