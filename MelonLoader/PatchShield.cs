...
public class PatchShield : Attribute //Naming violation?
{
    ...
    {
        var patchProcessorType = typeof(PatchProcessor);
        var patchShieldType = typeof(PatchShield);
        ...
        try
        {
            var unpatchMethod = AccessTools.Method(patchShieldType, "PatchMethod_PatchProcessor_Unpatch").ToNewHarmonyMethod();
            foreach (var method in patchProcessorType.GetMethods(BindingFlags.Public | BindingFlags.Instance).Where(x => x.Name.Equals("Unpatch")))
