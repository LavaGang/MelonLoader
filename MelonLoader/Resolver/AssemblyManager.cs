...
{
    internal static Dictionary<string, AssemblyResolveInfo> InfoDict = [];
    ...
    {
        if (InfoDict.TryGetValue(name, out var resolveInfo))
            return resolveInfo;
        lock (InfoDict)
    ...
    private static Assembly Resolve(string requested_name, Version requested_version, bool is_preload)
    {
        ...
        var resolveInfo = GetInfo(requested_name);

        // Resolve the Information Object
        var assembly = resolveInfo.Resolve(requested_version);
    ...
    internal static void LoadInfo(Assembly assembly)
    {
        ...
        // Get Resolve Information Object
        var resolveInfo = GetInfo(assemblyName.Name);
    ...
    {
        var requested_version = new Version(major, minor, build, revision);
