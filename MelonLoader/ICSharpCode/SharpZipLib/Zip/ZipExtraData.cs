...
public class ExtendedUnixData : ITaggedData
{
    ...
    public byte[] GetData()
    {
        ...
        if ((_flags & Flags.ModificationTime) != 0)
        {
            var span = _modificationTime - new DateTime(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc);
...
public sealed class ZipExtraData : IDisposable
{
    ...
    public bool Find(int headerID)
    {
        ...
        var result = (localTag == headerID) && ((_index + localLength) <= _data.Length);
    ...
    {
        if (headerID is > ushort.MaxValue or < 0)
        {
    ...
    public int ReadInt()
    {
        ...
        var result = _data[_index] + (_data[_index + 1] << 8) +
