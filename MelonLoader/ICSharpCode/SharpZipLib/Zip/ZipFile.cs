...
public class ZipFile : IEnumerable, IDisposable
{
    ...
    public string Password
    {
        set
        {
            key = string.IsNullOrEmpty(value) ? null : PkzipClassic.GenerateKeys(ZipStrings.ConvertToArray(value));
    ...
    {
        return isDisposed_ ? throw new ObjectDisposedException("ZipFile") : (IEnumerator)new ZipEntryEnumerator(entries_);
    }
    ...
    private long TestLocalHeader(ZipEntry entry, HeaderTest tests)
    {
        lock (baseStream_)
        {
            ...
            if (testHeader)
            {
                if (extractVersion is <= 63 and   // Ignore later versions as we dont know about them..
                not 10 and
                not 11 and
                not 20 and
                not 21 and
                ...
                var localName = ZipStrings.ConvertToStringExt(localFlags, nameData);
    ...
    public void CommitUpdate()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    var theComment = (newComment_ != null) ? newComment_.RawComment : ZipStrings.ConvertToArray(comment_);
    ...
    private void AddUpdate(ZipUpdate update)
    {
        ...
        var index = FindExistingUpdate(update.Entry.Name, isEntryName: true);
    ...
    {
        if (compressionMethod is not CompressionMethod.Deflated and not CompressionMethod.Stored and not CompressionMethod.BZip2)
        {
    ...
    */
    ...
    private void WriteLocalEntryHeader(ZipUpdate update)
    {
        ...
        var name = ZipStrings.ConvertToArray(entry.Flags, entry.Name);
    ...
    private int WriteCentralDirectoryHeader(ZipEntry entry)
    {
        ...
        var rawComment = (entry.Comment != null) ? Encoding.ASCII.GetBytes(entry.Comment) : Empty.Array<byte>();
    ...
    private int FindExistingUpdate(string fileName, bool isEntryName = false)
    {
        ...
        var convertedName = !isEntryName ? GetTransformedFileName(fileName) : fileName;
    ...
    {
        var skipOver = false || update.Entry.Offset == destinationPosition;
    ...
    private class UpdateComparer : IComparer<ZipUpdate>
    {
        ...
        public int Compare(ZipUpdate x, ZipUpdate y)
        {
            ...
            {
                var xCmdValue = (x.Command is UpdateCommand.Copy or UpdateCommand.Modify) ? 0 : 1;
                var yCmdValue = (y.Command is UpdateCommand.Copy or UpdateCommand.Modify) ? 0 : 1;
    ...
    private void ReadEntries()
    {
        ...
        for (ulong i = 0; i < entriesForThisDisk; i++)
        {
            ...
            var name = ZipStrings.ConvertToStringExt(bitFlags, buffer, nameLen);
