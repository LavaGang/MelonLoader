...
public class TarBuffer
{
    ...
    */
    ...
    public void WriteBlock(byte[] block)
    {
        ...
        if (block.Length != BlockSize)
        {
            var errorText = string.Format("TarBuffer.WriteBlock - block to write has length '{0}' which is not the block size of '{1}'",
    ...
    public void WriteBlock(byte[] buffer, int offset)
    {
        ...
        {
            var errorText = string.Format("TarBuffer.WriteBlock - record has length '{0}' with offset '{1}' which is less than the record size of '{2}'",
