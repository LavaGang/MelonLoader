...
internal static class CrcUtilities
{
    ...
    internal static uint[] GenerateSlicingLookupTable(uint polynomial, bool isReversed)
    {
        ...
        var one = isReversed ? 1 : (1U << 31);

        for (var i = 0; i < 256; i++)
        {
            var res = (uint)(isReversed ? i : i << 24);
            for (var j = 0; j < SlicingDegree; j++)
            {
                for (var k = 0; k < 8; k++)
                {
    ...
    {
        var x1 = (byte)((byte)(checkValue >> 24) ^ input[offset]);
        var x2 = (byte)((byte)(checkValue >> 16) ^ input[offset + 1]);
        var x3 = (byte)((byte)(checkValue >> 8) ^ input[offset + 2]);
        var x4 = (byte)((byte)checkValue ^ input[offset + 3]);
    ...
    {
        var x1 = (byte)((byte)checkValue ^ input[offset]);
        var x2 = (byte)((byte)(checkValue >>= 8) ^ input[offset + 1]);
        var x3 = (byte)((byte)(checkValue >>= 8) ^ input[offset + 2]);
        var x4 = (byte)((byte)(checkValue >>= 8) ^ input[offset + 3]);
    ...
    {
        uint result;
        var a1 = crcTable[x1 + 3840] ^ crcTable[x2 + 3584];
        var a2 = crcTable[x3 + 3328] ^ crcTable[x4 + 3072];
