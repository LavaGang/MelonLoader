...
internal static class ProcessFix
{
    ...
    internal static void Install()
    {
        ...
        var processFixType = typeof(ProcessFix);
        ...
        {
            MelonLogger.Warning($"ProcessFix Exception: {ex}");
        }
    }
    ...
    {
        var intPtr = __instance.MainWindowHandle;
        if (intPtr == IntPtr.Zero)
            __result = string.Empty;
        else
        {
            var capacity = GetWindowTextLength(new HandleRef(__instance, intPtr)) * 2;
    ...
    internal class MainWindowFinder
    {
        ...
        internal IntPtr FindMainWindow(int processId)
        {
            Handle = (IntPtr)0;
            ID = processId;
            var enumThreadWindowsCallback = new EnumThreadWindowsCallback(EnumWindowsCallback);
