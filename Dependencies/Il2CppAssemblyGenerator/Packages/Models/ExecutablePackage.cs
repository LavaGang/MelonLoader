...
internal class ExecutablePackage : PackageBase
{
    ...
    internal bool Execute(string[] args, bool parenthesize_args = true, Dictionary<string, string> environment = null)
    {
        ...
        try
        {
            ...
            var tempFolder = Path.Combine(Path.GetDirectoryName(ExeFilePath),
            ...
            var processStartInfo = new ProcessStartInfo(ExeFilePath, parenthesize_args
                ?
    ...
    private static void OutputStream(object sender, DataReceivedEventArgs e)
    {
        if (e.Data == null)
            ResetEvent_Output.Set();
        else
            Core.Logger.Msg(e.Data);
    }
    private static void ErrorStream(object sender, DataReceivedEventArgs e)
    {
        if (e.Data == null)
            ResetEvent_Error.Set();
        else
            Core.Logger.Error(e.Data);
    }
